...
public class SessionTest extends TestCase {
    ...
    public void testGetCurrentAverage() {
        ...
        assertEquals(0, session.getCurrentAverage()); // if total is >
                                                        // attempts, return zero
        ...
        session.setSolvesOnScreen(2);
        assertEquals(0, session.getCurrentAverage()); // or if total -
                                                        // (best+worst) <1
        session.getAttempts().add(new Solve(146, ResultType.SOLVED));
        session.getAttempts().add(new Solve(146, ResultType.SOLVED));
        session.getAttempts().add(new Solve(146, ResultType.SOLVED));
    ...
    public void testGetFastestListOfSolve() {
        ...
        assertEquals(98, session.getSpeedOrderedAttempts().get(
                session.getAttempts().size() - 1).getTime());
    }
