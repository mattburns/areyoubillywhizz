...
public class Session {
	...
	private enum SortFields {DATE, TIME};
	
	public int getCurrentAverage() {
		if (attempts.size() < solvesOnScreen || solvesOnScreen - (discardBest+discardWorst) < 1) {
			return 0;
		}
		
		int average = 0;
		int firstIndex = attempts.size() - solvesOnScreen;
		List<Solve> currentSolves = getDateOrderedAttempts().subList(firstIndex, attempts.size());
		
		List<Solve> sortedCurrentSolves = sortByTime(true, currentSolves);
		
		List<Solve> trimmedSortedCurrentSolves = sortedCurrentSolves.subList(discardBest, (sortedCurrentSolves.size() - discardWorst));
	...
	public static List<Solve> sortByTime(boolean fastestToSlowest, List<Solve> unsorted) {
		return sortSolves(fastestToSlowest, SortFields.TIME, unsorted);
	}
	
	public static List<Solve> sortByDate(boolean oldestToNewest, List<Solve> unsorted) {
		return sortSolves(oldestToNewest, SortFields.DATE, unsorted);
	}
	
	private static List<Solve> sortSolves(boolean smallestToBiggest, SortFields field, List<Solve> unsorted) {
			...
			for (Solve solve : unsorted) {
				if (!sorted.contains(solve)) {
					if (extremeSolveNotAdded == null) {
						...
						switch (field) {
						case DATE:
							if ((solve.getDate().before(extremeSolveNotAdded.getDate()) && smallestToBiggest) ||
								(solve.getDate().after(extremeSolveNotAdded.getDate()) && !smallestToBiggest)) {
									extremeSolveNotAdded = solve;
							}
						break;
						case TIME:
							if ((solve.getTime() > extremeSolveNotAdded.getTime() && smallestToBiggest) ||
							(solve.getTime() < extremeSolveNotAdded.getTime() && !smallestToBiggest)) {
								extremeSolveNotAdded = solve;
							}
						break;
						}
